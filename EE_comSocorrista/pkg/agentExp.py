...
class AgentExp:
    def __init__(self, model, configDict, base):
        """ 
        ...
        self.plan = ExpPlan(model.rows, model.columns, self.prob.goalState, initial, self.base, self.model, "goal", self.mesh)
    ...
    def deliberate(self):
        ...
        base = self.base
        current = (self.currentState.row, self.currentState.col)
        ...
        crrntState = self.positionSensor()
        if self.prob.goalTest(crrntState) and self.tl < self.maxT - 8:
    ...
    def victimVitalSignalsSensor(self, victimId):
        ...
        #self.tl -= 2 # custo de ver os sinais vitais da vitima
